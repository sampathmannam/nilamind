// Nila's Voice — companion personality response templates + offline companion reply builder.
...
// This module is the canonical home of Nila's voice. nilaVoice.ts re-exports it for
// backward compatibility with callers that still import from the old path.
...
/** Context used to choose a warm, safe offline companion reply when the on-device brain
 *  isn't reachable (cold load / skipped download / OOM / web without bridge). The reply is
 *  deterministic (no model) and intentionally LEFT UNGATED by the §9 output gate (see
 *  nilaSafetyGate.ts INVARIANT 3), so it must be safe by construction:
 *  - companion, never therapist (no "therapy / treat / diagnose / cure");
 *  - never validates or colludes with delusional / grandiose / impulsive content;
 *  - for crisis it points to real help + grounding, never minimizes or reassures away risk. */
export type OfflineCompanionContext = {
  isCrisis?: boolean;
  recentMoodAvg?: number | null;
  streakDays?: number;
  isReturning?: boolean;
...
/** Builds a warm, safe companion-voice message for the offline path. Reuses the curated
 *  voice templates so the offline experience sounds like Nila, not an error wall. */
export function buildOfflineCompanionReply(
  ctx: OfflineCompanionContext = {},
  seed?: number
): string {
  const scenario = detectScenario({
    timeOfDay: ctx.timeOfDay ?? "afternoon",
    recentMoodAvg: ctx.recentMoodAvg ?? null,
    checkedInToday: false,
    streakDays: ctx.streakDays ?? 0,
    sleepHours: null,
    isCrisis: Boolean(ctx.isCrisis),
    hasRecentEpisode: false,
    isReturning: Boolean(ctx.isReturning),
  });
  const { message } = buildNilaMessage(
    {
      timeOfDay: ctx.timeOfDay ?? "afternoon",
      recentMoodAvg: ctx.recentMoodAvg ?? null,
      checkedInToday: false,
  ...
  if (scenario === "crisis_detected" || Boolean(ctx.isCrisis)) {
    return `${message}\n\nIf you're in immediate danger, please reach out to someone you trust or a local emergency service. The grounding and breathing tools just below are here for you right now. 💙`;
  }
