...
describe("crisisSignalForSend (2026-07-12 Wave 3 — two-tier crisis surface)", () => {
  it("delegates to detectCrisisSignal: a keyword hit returns source:'keyword'", async () => {
    expect(await crisisSignalForSend("I want to die")).toEqual({ hit: true, source: "keyword" });
  });
  it("delegates to detectCrisisSignal: no hit returns hit:false, source:null", async () => {
    expect(await crisisSignalForSend("I'm really anxious about work")).toEqual({ hit: false, source: null });
  });
});
