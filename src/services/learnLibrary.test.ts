...
describe("searchLearn — one §9-gated search across Skills + Understand + Why", () => {
  it("empty query returns items from all three sources (browse mode)", async () => {
    const sources = new Set((await searchLearn("")).map((r) => r.source));
    expect(sources.has("skill")).toBe(true);
    expect(sources.has("understand")).toBe(true);
    expect(sources.has("why")).toBe(true);
  });
  it("a topical query returns matching results tagged by source", async () => {
    const r = await searchLearn("behavioural activation");
    expect(r.length).toBeGreaterThan(0);
    expect(r.some((x) => x.source === "why" && /activation/i.test(x.title))).toBe(true);
  });
  it("every result carries id, title, snippet, source", async () => {
    for (const x of await searchLearn("")) {
  ...
  it("§9-gates: a crisis query returns NO library content (UI surfaces help instead)", async () => {
    expect(await searchLearn("i want to kill myself")).toEqual([]);
  });
});
