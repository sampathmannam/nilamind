...
export default function PactNoticeCard({ notice, onDismiss }: { notice: PactNotice; onDismiss: () => void }) {
  ...
  return (
    <div className="w-full rounded-xl bg-warn/10 border border-warn/30 p-3 space-y-2" id="pact-notice-card">
      <div className="flex items-start justify-between gap-2">
        ...
        <button
          ...
          className="shrink-0 text-warn/70 hover:text-amber-200 cursor-pointer p-1 -m-1"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
      ...
      {sms && (
        <a
          ...
          className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-warn/20 border border-warn/40 text-amber-100 text-sm font-medium hover:bg-warn/30 transition-colors cursor-pointer"
        >
