...
export default function NudgeRail({
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1">
            ...
            <div className="flex gap-2 mt-2">
              <button onClick={onOpenSafetyPlan} className="px-3 py-2 rounded-lg bg-warn/20 hover:bg-warn/30 text-warn font-medium transition-colors cursor-pointer min-h-[44px] focus-ring">Review plan</button>
              <button onClick={onCompleteReview} className="px-3 py-2 rounded-lg hover:bg-warn/15 text-warn/80 transition-colors cursor-pointer min-h-[44px] focus-ring">Looks good</button>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="w-full px-3 py-2 rounded-xl bg-accent/10 border border-accent/30 text-accent-hi text-xs" id="safety-plan-followup-card">
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1">
            ...
            <div className="flex gap-2 mt-2">
              <button onClick={onOpenSafetyPlan} className="px-3 py-2 rounded-lg bg-accent/20 hover:bg-accent/30 text-accent-hi font-medium transition-colors cursor-pointer min-h-[44px] focus-ring">Review plan</button>
              <button onClick={onCompleteFollowUp} className="px-3 py-2 rounded-lg hover:bg-accent/15 text-accent-hi/80 transition-colors cursor-pointer min-h-[44px] focus-ring">Done</button>
            </div>
          </div>
        </div>
      </div>
  ...
  if (visibleNudgeIds.has("safetyPlan") && safetyPlanCard === "calm" && calmSafetyNudge?.show) {
    items.push({ key: "safetyPlan-calm", el: (
      <div key="calm-safety-plan-nudge" className="w-full px-3 py-2 rounded-xl bg-accent/10 border border-accent/30 text-accent-hi text-xs" id="calm-safety-plan-nudge-card">
        <div className="flex items-start gap-2">
          <ShieldCheck className="w-4 h-4 text-accent mt-0.5 shrink-0" />
          <div className="flex-1">
            <p className="font-medium">{calmSafetyNudge.label}</p>
            <div className="flex gap-2 mt-2">
              <button onClick={onOpenSafetyPlan} className="px-3 py-2 rounded-lg bg-accent/20 hover:bg-accent/30 text-accent-hi font-medium transition-colors cursor-pointer min-h-[44px] focus-ring">Fill it in</button>
              <button onClick={onDismissCalm} className="px-3 py-2 rounded-lg hover:bg-accent/15 text-accent-hi/80 transition-colors cursor-pointer min-h-[44px] focus-ring">Not now</button>
            </div>
          </div>
        </div>
      </div>
  ...
  if (visibleNudgeIds.has("sleep") && sleepProdromeNudge) {
    items.push({ key: "sleep", el: (
      <div key="sleep-prodrome" className="w-full px-3 py-2 rounded-xl bg-warn/10 border border-warn/30 text-warn text-xs" id="sleep-prodrome-card">
        <div className="flex items-start gap-2">
          <Moon className="w-4 h-4 text-warn mt-0.5 shrink-0" />
          <div className="flex-1">
            <p className="font-medium">Sleep has been short lately</p>
            <p className="text-warn/70 mt-0.5">{sleepProdromeNudge.detail}</p>
            <div className="flex gap-2 mt-2">
              <button onClick={() => onOpenWindDown?.()} className="px-3 py-2 rounded-lg bg-warn/20 hover:bg-warn/30 text-warn font-medium transition-colors cursor-pointer min-h-[44px] focus-ring">Wind down</button>
              <button onClick={onDismissSleep} className="px-3 py-2 rounded-lg hover:bg-warn/15 text-warn/80 transition-colors cursor-pointer min-h-[44px] focus-ring">Not now</button>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="flex-1">
            ...
            {jitaiNudge.suggestedTool && (
              <button
                ...
                className="px-3 py-2 rounded-lg bg-accent/20 hover:bg-accent/30 text-accent-hi font-medium transition-colors cursor-pointer mt-2 min-h-[44px] focus-ring"
              >
