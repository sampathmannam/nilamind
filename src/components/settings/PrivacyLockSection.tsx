      ...
      <div>
        ...
        <p className="text-[11px] text-ink-muted mt-1 leading-relaxed">
          ...
          {passthrough ? (
            ...
            are stored <span className="text-warn">unencrypted</span> here.</>
          ) : (
      ...
      <div className="border border-line rounded-xl p-3 bg-page space-y-3">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <ShieldCheck className={`w-4 h-4 ${pinOn ? "text-success" : "text-slate-600"}`} />
          ...
          {mode === "idle" && !passthrough && (
            pinOn ? (
              ...
              <button onClick={() => setMode("setting")} id="settings-set-pin" className="text-[11px] font-semibold text-accent-hi hover:text-accent-hi cursor-pointer">Set up</button>
            )
          )}
        </div>
