  ...
  return (
    <div className="space-y-5 max-w-md mx-auto" id="episode-marker-screen">
      ...
      <header className="space-y-2">
        ...
        <p className="text-xs text-slate-400 leading-relaxed">{t("em_intro")}</p>
      </header>
      ...
      <div className="glass rounded-2xl p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <label className="space-y-1">
            <span className="text-[11px] text-slate-400">{t("em_from")}</span>
            <input
              type="date"
              value={from}
              onChange={(e) => setFrom(e.target.value)}
              className="w-full bg-page border border-slate-800 rounded-lg px-2 py-1.5 text-sm text-slate-200"
            />
          </label>
          <label className="space-y-1">
            <span className="text-[11px] text-slate-400">{t("em_to")}</span>
            <input
              type="date"
              value={to}
        ...
        <label className="space-y-1">
          <span className="text-[11px] text-slate-400">{t("em_note_optional")}</span>
          <input
            type="text"
            value={note}
          ...
          sorted.map((m) => (
            <div key={m.id} className="bg-slate-800/50 border border-slate-700 rounded-xl p-3 space-y-1">
              <div className="flex items-center justify-between">
                <span className="text-sm font-semibold text-slate-100">{t(phaseKey(m.phase))}</span>
                <span className="text-[11px] text-slate-500">
                  {m.startDate === m.endDate ? m.startDate : `${m.startDate} – ${m.endDate}`}
                </span>
              </div>
              {m.note && <p className="text-[11px] text-slate-400">{m.note}</p>}
            </div>
          ))
        )}
      </div>
