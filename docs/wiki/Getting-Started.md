...
- **~1.5 GB of free storage** and enough RAM to hold the model. The default brain (Qwen2.5-1.5B, ~1.1 GB) runs comfortably on most recent phones; the optional reference 4B model (~2.5 GB) is memory-fragile on low-RAM devices, so a mid-range or flagship is recommended if you side-load it.
...
2. **Download Nila's brain.** On first launch the app offers to download its on-device language model (~1.1 GB). It confirms before starting (use Wi-Fi), streams it to disk, **verifies its integrity**, and only then activates it. After that, the app works fully offline. See [Model Provisioning](Model-Provisioning.md).
